...
export function AccountsMenuClient({ accounts }: AccountsMenuClientProps) {
  ...
  const accountsByInstitution = accounts.reduce((acc: Record<string, Account[]>, account: Account) => {
  ...
  useEffect(() => {
    if (pathname.startsWith("/accounts/") || pathname === "/connect-account") {
      ...
      Object.entries(accountsByInstitution).forEach(([institutionName, institutionAccounts]) => {
        const hasActiveAccount = institutionAccounts.some(
          (account) => pathname === `/accounts/${account.id}`
        );
        if (hasActiveAccount) {
      ...
      {isOpen && (
        <SidebarMenuSub>
          ...
          <SidebarMenuSubItem>
            <SidebarMenuSubButton
              asChild
              isActive={pathname === "/connect-account"}
            >
            ...
            .map(([institutionName, institutionAccounts]) => (
              <SidebarMenuSubItem key={institutionName}>
                <SidebarMenuSubButton
                  ...
                  isActive={institutionAccounts.some(
                    (account) => pathname === `/accounts/${account.id}`
                  )}
                >
