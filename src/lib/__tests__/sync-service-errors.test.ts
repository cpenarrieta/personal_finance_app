...
describe("Sync Service Error Handling", () => {
  ...
  describe("Plaid API Errors", () => {
    it("should throw error when Plaid API fails during transaction sync", async () => {
      ...
      // Act & Assert
      await expect(syncItemTransactions(itemId, accessToken, "cursor")).rejects.toThrow("ITEM_LOGIN_REQUIRED")
    })
    ...
    it("should throw error when Plaid API fails during historical fetch", async () => {
      ...
      // Act & Assert
      await expect(syncItemTransactions(itemId, accessToken, null)).rejects.toThrow("INSTITUTION_DOWN")
    })
    ...
    it("should throw error when Plaid API fails during investment sync", async () => {
      ...
      // Act & Assert
      await expect(syncItemInvestments(itemId, accessToken)).rejects.toThrow("ITEM_NOT_FOUND")
    })
    ...
    it("should throw error when investment transactions fetch fails", async () => {
      ...
      // Act & Assert
      await expect(syncItemInvestments(itemId, accessToken)).rejects.toThrow("RATE_LIMIT_EXCEEDED")
    })
  })
  ...
  describe("Database Errors", () => {
    it("should throw error when transaction upsert fails", async () => {
      ...
      // Act & Assert
      await expect(syncItemTransactions(itemId, accessToken, "cursor")).rejects.toThrow("Database connection failed")
    })
    ...
    it("should throw error when account upsert fails", async () => {
      ...
      // Act & Assert
      await expect(syncItemTransactions(itemId, accessToken, "cursor")).rejects.toThrow("Unique constraint violation")
    })
    ...
    it("should throw error when transaction delete fails", async () => {
      ...
      // Act & Assert
      await expect(syncItemTransactions(itemId, accessToken, "cursor")).rejects.toThrow("Foreign key constraint failed")
    })
    ...
    it("should throw error when security upsert fails", async () => {
      ...
      // Act & Assert
      await expect(syncItemInvestments(itemId, accessToken)).rejects.toThrow("Database write failed")
    })
    ...
    it("should throw error when holding upsert fails", async () => {
      ...
      // Act & Assert
      await expect(syncItemInvestments(itemId, accessToken)).rejects.toThrow("Decimal precision error")
    })
  })
  ...
  describe("Network Errors", () => {
    it("should throw error on network timeout during transaction sync", async () => {
      ...
      // Act & Assert
      await expect(syncItemTransactions(itemId, accessToken, "cursor")).rejects.toThrow("ETIMEDOUT")
    })
    ...
    it("should throw error on network timeout during investment sync", async () => {
      ...
      // Act & Assert
      await expect(syncItemInvestments(itemId, accessToken)).rejects.toThrow("ECONNREFUSED")
    })
  })
  ...
  describe("Invalid Data Handling", () => {
    it("should handle missing account_id in transaction", async () => {
      ...
      // Act & Assert
      await expect(syncItemTransactions(itemId, accessToken, "cursor")).rejects.toThrow()
    })
