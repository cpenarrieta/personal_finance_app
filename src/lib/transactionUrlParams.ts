 ...
 */
export function transactionFiltersToUrlParams(
  filters: TransactionFiltersFromUrl
): URLSearchParams {
  ...
  if (
    ...
    params.set(
      "subcategoryId",
      Array.from(filters.selectedSubcategoryIds).join(",")
    );
  }
  ...
  if (filters.excludedCategoryIds && filters.excludedCategoryIds.size > 0) {
    params.set(
      "excludeCategory",
      Array.from(filters.excludedCategoryIds).join(",")
    );
  }
...
function isValidDateRange(value: string): value is DateRange {
  return ["all", "last30", "last90", "thisMonth", "lastMonth", "custom"].includes(
    value
  );
}
