...
async function cleanupDuplicates() {
  ...
  for (const [instId, instItems] of itemsByInstitution.entries()) {
    if (instItems.length > 1) {
      ...
      const sorted = instItems
        .map((item: typeof items[0], idx: number): MappedItem => ({
          item,
  ...
  console.log(
    "⚠️  Make sure to review the list above before confirming\n"
  );
  ...
  for (const item of duplicatesToDelete) {
    ...
    for (const account of item.accounts) {
      try {
        ...
        console.error(
          `     This account likely has transactions. Please review manually.`
        );
