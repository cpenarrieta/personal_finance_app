...
async function deleteItemAndData() {
  ...
  const totalTx = item.accounts.reduce((sum: number, acc: typeof item.accounts[0]) => sum + acc._count.transactions, 0);
  const totalHoldings = item.accounts.reduce((sum: number, acc: typeof item.accounts[0]) => sum + acc._count.holdings, 0);
  const totalInvTx = item.accounts.reduce((sum: number, acc: typeof item.accounts[0]) => sum + acc._count.investmentTransactions, 0);
